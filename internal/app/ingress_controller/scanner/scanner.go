...
func main() {
	//Ensures that we do not have to run the kubectl proxy with TLS credentials
	exec.Command("kubectl", "proxy", "--insecure-skip-tls-verify").Start()
	GetServices()
}

// GetServices gets all of the services in our cluster through the API and configures this information
func GetServices() {
	//This is able to send requests to the API in order to retreive information needed from this
...
// GetIngress gets all of the ingress specifications using the API and
// func GetIngress() {
// 	apiResp, err := exec.Command()
// 	if err != nil {
// 		fmt.Println(err)
// 	}
// }
